...
- **True baseline (verified this session, branch `overhaul/full-refresh`): CORRECTED BASELINE: 299 runs, 0 failures, 0 errors, 9 skips (the earlier "1F/3E pre-existing breakage" reading was a polluted measurement taken against a concurrent implementer's uncommitted tree — the committed branch is fully green; do NOT carry forward any failures)
